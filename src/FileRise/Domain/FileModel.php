...
require_once dirname(__DIR__, 3) . '/src/lib/ACL.php';
...
/* ignore */
        ...
        $isUnsafeEntryPath = function (string $entry): bool {
            $e = str_replace('\\', '/', $entry);
            if ($e === '' || str_contains($e, "\0")) {
                return true;
            }
            if (str_starts_with($e, '/')) {
                return true;                 // absolute nix path
            }
            if (preg_match('/^[A-Za-z]:[\\/]/', $e)) {
                return true;      // Windows drive
            }
            if (str_contains($e, '../') || str_contains($e, '..\\')) {
                return true;
            }
            return false;
        };
        ...
        $validEntrySubdirs = function (string $entry): bool {
            ...
            foreach ($dirs as $d) {
                if ($d === '' || !preg_match(REGEX_FOLDER_NAME, $d)) {
                    return false;
                }
            }
            return true;
        ...
        $isRarArchiveName = function (string $name): bool {
            ...
            if (preg_match('/\\.r\\d{2}$/i', $lower)) {
                return true;
            }
            if (preg_match('/\\.part\\d+\\.rar$/i', $lower)) {
                return true;
            }
            return false;
        };
        ...
        foreach ($files as $archiveName) {
            ...
            $flushEntry = function () use (
                ...
                if ($name === '' || preg_match('/[\\r\\n]/', $name)) {
                    $unsafe = true;
                    if ($unsafeReason === '') {
                        $unsafeReason = 'Archive contains invalid entry names.';
                    }
                    return;
                }
                if ($isUnsafeEntryPath($name)) {
...
/* ignore */
    ...
    public static function getFileList(string $folder, array $options = []): array
    {
        ...
        if (!defined('LISTING_CONTENT_BYTES_MAX')) {
            define('LISTING_CONTENT_BYTES_MAX', 8192);          // 8 KB snippet
        }
        if (!defined('INDEX_TEXT_BYTES_MAX')) {
